...
func CreateUserPoints(req CreateUserBody) ([]*pb.PointStruct, error) {
	...
	_, isRegistered, _ := GetUserData(req.PubKey)
	if isRegistered {
		return nil, errors.New(USER_ALREADY_EXISTS_MSG)
	}
...
// An abstract function to create a user
func CreateUser(req CreateUserBody) (string, int) {
	user, err := CreateUserPoints(req)
	if err != nil {
		return err.Error(), BAD_REQUEST
	}

	err, _ = UpsertPoints(user)
	if err != nil {
		return err.Error(), INTERNAL_SERVER_ERROR
	}

	return USER_CREATED_MSG, CREATED
...
func parseSearchRes(res *pb.SearchResponse) UserSearchResult {
	...
	for _, point := range res.Result {
		for key, value := range point.Payload {
			switch key {
			case "username":
				searchStruct.Username = value.GetStringValue()
			case "email":
				searchStruct.Email = value.GetStringValue()
			}
		}
	}
...
func SaveClientData(data map[string]string) error {
	_, err := clientsCollection.InsertOne(context.TODO(), data)
	return err
}

// Get client data by clientId from MongoDB
func GetClientData(clientId string) (map[string]string, error) {
	var result map[string]string
	err := clientsCollection.FindOne(context.TODO(), bson.M{"clientId": clientId}).Decode(&result)
	return result, err
}
